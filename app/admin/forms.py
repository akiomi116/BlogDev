...
class UserForm(FlaskForm):
    ...
    def __init__(self, *args, **kwargs):
        self.obj = kwargs.get('obj', None) # 既存オブジェクト (ユーザー) が渡された場合
        super(UserForm, self).__init__(*args, **kwargs)
        
    def validate_username(self, username):
        # ユーザー編集時、現在のユーザー名が変更されていない場合は検証をスキップ
        if self.obj and username.data == self.obj.username:
            return

        # 新しいユーザー名がすでに存在するかチェック
        user = User.query.filter_by(username=username.data).first()
        if user:
            raise ValidationError('そのユーザー名はすでに使われています。別のユーザー名を選んでください。')

    def validate_email(self, email):
        # ユーザー編集時、現在のメールアドレスが変更されていない場合は検証をスキップ
        if self.obj and email.data == self.obj.email:
            return

        # 新しいメールアドレスがすでに登録されているかチェック
        user = User.query.filter_by(email=email.data).first()
        if user:
            raise ValidationError('そのメールアドレスはすでに登録されています。')
